...
namespace RxClock.Tests.PlayMode.Clock
{
    ...
    {
        [UnityTest]
        public IEnumerator CountSecondsAutomaticallyFromCreation() => UniTask.ToCoroutine(async () =>
        {
        ...
        private TimeSpan GivenSeconds(int seconds) => TimeSpan.FromSeconds(seconds);
        
        private async UniTask GivenSecondsPassed(TimeSpan seconds) => await UniTask.Delay(seconds);
        ...
        private void ThenTimeSpanDifferenceShouldBe(DateTime time1, DateTime time2, TimeSpan interval)
        {
            ...
            difference.Should().BeCloseTo(interval, precision:acceptableError);
        }
    }
}